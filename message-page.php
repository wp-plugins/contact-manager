...
else { echo "var anchor = window.location.hash;\n";
foreach ($modules['message'] as $key => $value) {
...
if (!isset($_GET['id'])) { $string2 = ''; foreach ($add_message_fields as $field) { $string2 .= '"'.$field.'", '; } }
...
jQuery.post("'.CONTACT_MANAGER_URL.'?action=fill-admin-page-fields&page='.$_GET['page'].(isset($_GET['id']) ? '&id='.$_GET['id'] : '').'&time='.$current_time.'&key='.md5(AUTH_KEY).'", data, function(data) {
'.((!isset($_GET['id'])) ? 'fields = fields.concat(['.substr($string2, 0, -2).']);'."\n" : '').
'for (i = 0; i < fields.length; i++) {
